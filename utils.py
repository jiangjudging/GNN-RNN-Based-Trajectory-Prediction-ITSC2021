...
def maskedNLL(y_pred, y_gt, mask):
    ...
    out = 0.5 * torch.pow(ohr, 2) * (torch.pow(sigX, 2) * torch.pow(x - muX, 2) + torch.pow(sigY, 2) * torch.pow(y - muY, 2) -
                                     2 * rho * torch.pow(sigX, 1) * torch.pow(sigY, 1) * (x - muX) * (y - muY)) - torch.log(sigX * sigY * ohr) + 1.8379
        ...
        for k in range(num_lon_classes):
            for l in range(num_lat_classes):
                ...
                out = -(0.5 * torch.pow(ohr, 2) *
                        (torch.pow(sigX, 2) * torch.pow(x - muX, 2) + 0.5 * torch.pow(sigY, 2) * torch.pow(y - muY, 2) -
                         rho * torch.pow(sigX, 1) * torch.pow(sigY, 1) * (x - muX) * (y - muY)) - torch.log(sigX * sigY * ohr) + 1.8379)
                # If we represent likelihood in m^(-1):
                # out = -(0.5 * torch.pow(ohr, 2) * (torch.pow(sigX, 2) * torch.pow(x - muX, 2) + torch.pow(sigY, 2) * torch.pow(y - muY, 2) - 2 * rho * torch.pow(sigX, 1) * torch.pow(sigY, 1) * (x - muX) * (y - muY)) - torch.log(sigX * sigY * ohr) + 1.8379 - 0.5160)
...
def maskedMSEFinalTest(y_pred, y_gt, mask):
    '''
    给最终测试使用，分别输出x和y方向的误差
    '''
    acc = torch.zeros_like(mask)
    muX = y_pred[:, :, 0]
    muY = y_pred[:, :, 1]
    ...
    y = y_gt[:, :, 1]
    out = torch.pow(x - muX, 2) + torch.pow(y - muY, 2)
    lossval = torch.zeros_like(mask)
    lossval[:, :, 0] = out
    lossval = torch.sum(lossval[:, :, 0], dim=1)
    # print(out.size())
    acc[:, :, 0] = torch.pow(x - muX, 2)
    acc[:, :, 1] = torch.pow(y - muY, 2)
    acc = acc * mask
    # print('acc', acc)
    x_lossVal = torch.sum(acc[:, :, 0], dim=1)
    y_lossVal = torch.sum(acc[:, :, 1], dim=1)
    counts = torch.sum(mask[:, :, 0], dim=1)
    # print(x_lossVal.size())
    return x_lossVal, y_lossVal, lossval, counts
